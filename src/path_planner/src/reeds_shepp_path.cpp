...
#include "path_planner/math_utils.h"
...
bool ReedShepp::ShortestRSP(const std::shared_ptr<Node3d> start_node,
  ...
  if (!GenerateRSPs(start_node, end_node, &all_possible_paths)) {
    ...
    std::cout << "Succeed to generate different combination of Reed Shepp paths"<<std::endl;
  }
  ...
  if (!GenerateLocalConfigurations(start_node, end_node,
    ...
    std::cout << "succeed to generate local configurations(x, y, phi) in SetRSP" << std::endl;
  }
  ...
  if (std::abs(all_possible_paths[optimal_path_index].x.back() -
      ...
      std::abs(all_possible_paths[optimal_path_index].y.back() -
               end_node->GetY()) > 1e-3 
      || std::abs(all_possible_paths[optimal_path_index].phi.back() -end_node->GetPhi()) > 0.17
      ) 
  {
...
bool ReedShepp::GenerateRSPs(const std::shared_ptr<Node3d> start_node,
  ...
  if (false) {
    ...
    if (!GenerateRSP(start_node, end_node, all_possible_paths)) {
      ...
      std::cout << "succeed to generate general profile of different RSPs"<<std::endl;
    }
  }
  return true;
}
...
bool ReedShepp::GenerateRSP(const std::shared_ptr<Node3d> start_node,
                            const std::shared_ptr<Node3d> end_node,
                            std::vector<ReedSheppPath>* all_possible_paths) {
  // 将起点平移至原点，并且和x轴对齐，除以转弯半径，将半径归一化到1
...
bool ReedShepp::GenerateLocalConfigurations(
  ...
  // std::cout<<"point_num is: "<<point_num<<std::endl;
  // std::cout<<shortest_path->total_length<<", "
  //   <<step_scaled<<", "
  //   <<shortest_path->segs_lengths.size()<<", "
  //   <<max_kappa_<<", "
  //   <<std::endl;
